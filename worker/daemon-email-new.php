...
if ($child_pid < 0) {
    ...
    ini_set('error_log', __DIR__ . DIRECTORY_SEPARATOR . 'error.email.log');
    fclose(STDIN);  $STDIN = fopen('/dev/null', 'r');
    fclose(STDOUT); $STDOUT = fopen(__DIR__ . DIRECTORY_SEPARATOR . 'email.log', 'ab');
    fclose(STDERR); $STDERR = fopen(__DIR__ . DIRECTORY_SEPARATOR . 'email.error.log', 'ab');
    ...
    $pdo_tickets = new PDO("mysql:host={$config_db_tickets['host']};port={$config_db_tickets['port']};dbname={$config_db_tickets['name']}", $config_db_tickets['user'], $config_db_tickets['password'], array(PDO::MYSQL_ATTR_INIT_COMMAND => "SET NAMES utf8",PDO::ATTR_PERSISTENT => true));
    $daemon = new Daemon();
    $daemon->run($pdo_tickets);
}
...
class Daemon
{
    ...
    const PID_FILE = __DIR__ . DIRECTORY_SEPARATOR . 'email.pid';
    ...
    public function run($pdo_tickets)
    {
        // Пока $stop_server не установится в TRUE, гоняем бесконечный цикл
        while (!$this->stop_server) {
            pcntl_signal_dispatch();  // обработка пришедших сигналов
            $this->launchJob($pdo_tickets);
            sleep(1);  // пауза просто для подстраховки
        }
    }

    protected function launchJob($pdo_tickets)
    {
        // подключаемся к БД и резервируем задачу для текущего процесса
        $sql = sprintf("UPDATE `db_job_queue` SET `status`=1, `detail`='daemon%s' WHERE `service` LIKE '%s' AND `status`=0 LIMIT 1", getmypid(), '%mail%');
        if(!(\Db::update($sql))) {
        ...
        $sql = "SELECT `id`, `data`, `company` FROM `db_job_queue` WHERE `detail`='daemon" . getmypid() . "'";
        ...
        if($result['company']=="SYNERGYWOMAN") {
            $stmtorders = $pdo_tickets->query("SELECT * FROM `orders` WHERE id = '" . $data["orderId"] . "'")->fetch();
            if($stmtorders[0]['status']!=4) {
                return FALSE;
            }
        }
        ...
        if (($data['dater'] != "") && ($data['dater'] !="null")) {
            ...
            if ($dateArr[4] !="") {
                $times = explode(":", $dateArr[4]);
                $unixTime = mktime($times[0], $times[1], 00, $month, $dateArr[0], $dateArr[2]);
                $dateEvent = $unixTime -10800;
                $date = new DateTime();
    			$currDate = $date->getTimestamp();
    			if ($dateEvent<$currDate+43200) {
                	$sql = "UPDATE `db_job_queue` SET `status` = 2, `detail` = :response, `dater` = :dater  WHERE id = :taskId";
                	$var = array(
                    	':response' => time(),
                    	':dater'    =>$dateEvent,
                    	':taskId' => $result['id']
            		...
            		$sql = "UPDATE `db_job_queue` SET `status` = 5, `detail` = :response, `dater` = :dater  WHERE id = :taskId";
                	$var = array(
                    	':response' => time(),
                    	':dater'    =>$dateEvent,
                    	':taskId' => $result['id']
