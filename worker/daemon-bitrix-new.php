...
if ($child_pid < 0) {
    ...
    fclose(STDOUT);
    $STDOUT = fopen(__DIR__ . DIRECTORY_SEPARATOR . 'bitrix.log', 'ab');
    fclose(STDERR);
    $STDERR = fopen(__DIR__ . DIRECTORY_SEPARATOR . 'bitrix.error.log', 'ab');
...
class Daemon
{
    ...
    const PID_FILE = __DIR__ . DIRECTORY_SEPARATOR . 'bitrix.pid';
    ...
    public function run()
    {
        ...
        $connection = new AMQPStreamConnection('msk1-dev19', '5672', 'admin', 'admin');
        $channelR7k12 = $connection->channel();
        $channelR7k12->queue_declare("r7k12", false, true, false, false);
 
        // Пока $stop_server не установится в TRUE, гоняем бесконечный цикл
        while (!$this->stop_server) {
            pcntl_signal_dispatch();  // обработка пришедших сигналов
            $this->launchJob($channelR7k12);
    ...
    protected function launchJob($channelR7k12)
    {
        global $log;
        
        // подключаемся к БД и резервируем задачу для текущего процесса
        $sql = sprintf("UPDATE `db_job_queue` SET `status`=1, `detail`='daemon%s' WHERE `service`='%s' AND `status`=0 LIMIT 1", getmypid(), 'bitrix24');
        if (!(\Db::update($sql))) {
   ...
   /*     if ($data_obj->landCode == 'sgf2017_en' || $data_obj->landCode == 'therealwolf' || $data_obj->landCode == 'sgf2017_en_university' || $data_obj->landCode == 'sgf2017_en_aev' || $data_obj->landCode == 'sgf2018_sid') {
            $t = date("d:m:y, H:m");
            $toSend = json_decode($result['data'], true);
            $toSend['time'] = $t;
            if ($data_obj->landCode == 'sgf2018_sid') {
                $toSend['sourceName'] = 'SIG_WEB';
            }
            $sentToGoogle = $this->sendToGoogleSheet($toSend);
        }*/

        if (isset($data_obj->bitrix) && $data_obj->bitrix == 'corp2') {
            $sendTo = B24_TEST_API;
        } elseif (isset($data_obj->bitrix) && $data_obj->bitrix == 'corpnew') {
            $sendTo = B24_NEW_API;
        } else {
            $sendTo = B24_API;
        }
	 // чтобы не хранить лишнюю копию $data в памяти
     ...
     //   $max_time = 300;
       ...
       // curl_setopt($ch, CURLOPT_TIMEOUT_MS, $max_time);
        ...
        if (isset($response->id)) {
            ...
            \Db::query($sql, $var);
            if (isset($data_obj->r7k12) && $data_obj->r7k12 != '') {
                $rabbitmessage = json_encode([[
                    ...
                    "status" => "NEW",
                    "shop" => " ",
                    "r7k12id" => "$data_obj->r7k12",
                ]]);
                $msg = new AMQPMessage($rabbitmessage);
                $channelR7k12->basic_publish($msg, '', 'r7k12');
            }
            if (isset($data_obj->roistat) && $data_obj->roistat != '') {
                $ch = curl_init();
                curl_setopt($ch, CURLOPT_URL, 'https://cloud.roistat.com/api/v1/project/clients/import?project=94484&key=9297234d4f894aa4a4e98eeaa179bd89');
                curl_setopt($ch, CURLOPT_POST, 1);
