        ...
        foreach ($json->messages as $message) {
            ...
            if ($phone != '79651388313') {
                if (isset($sel_Users['id']) && $sel_Users['id'] > 0) {
                    ...
                    $upd_User->execute(['id'=>$sel_Users['id'],'messages'=>json_encode($message,JSON_HEX_TAG | JSON_HEX_APOS  | JSON_HEX_AMP | JSON_UNESCAPED_UNICODE)]);
                } else {
                    $query = "INSERT INTO `lander`.`whatsapp` (`phone`,`messages`) VALUES (:phone,:messages)";
                    $ins_User = $pdo->prepare($query);
                    $ins_User->execute(['phone'=>$phone,'messages'=>json_encode($message)]);
                    $curl = curl_init();
