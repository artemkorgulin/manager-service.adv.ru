...
$category = 360;
$categoryName = strtolower(trim($_REQUEST['category']));
$product_id = 9420920;
switch ($categoryName) {
	case "econom":
		$category = 360;
...
$ticketsCount = isset($_REQUEST['tickets_count']) && $_REQUEST['tickets_count'] > 0 ? $_REQUEST['tickets_count'] * 1 : (isset($_REQUEST['synchro_count']) && $_REQUEST['synchro_count'] > 0 ? $_REQUEST['synchro_count'] * 1 : 1);
...
if (true) {
	...
	if (!isset($_REQUEST['payment-type']) && !isset($_REQUEST['company'])) {
		...
		<button class="cost-card__button modal-form__btn button button_payment-type form-button font-size-18 font-bold" name="payment-type-online">Оплата картой</button>
		<button class="cost-card__button modal-form__btn button button_payment-type form-button font-size-18 font-bold" name="payment-type-invoice">Выставить счет на оплату</button>
	...
	// шаг 2, выбран способ оплаты
		...
		if (isset($_REQUEST['payment-type-invoice'])) {
			...
			<div class="popup-form-inner">
				...
				<br><br>
			<button class="vertical-form-button modal-form__btn button">Выставить счет</button>
			</div>';
...
function createOrder($lead, $ticketsCount, $priceVariant, $promocode, $category, $invoice, $company, $productId, $applepay, $webmoney)
{
	...
	if ($promocode == '' && $category != 390) {
		$promocode = "SALEOP10";
	}

	if ($category == 390 || $category == 511) {
		$lang = 'ru';
	}
...
function getAdditionally($lead)
{
	...
	$additionally['shopId'] = ['name' => 'shopId', 'value' => 458119];
