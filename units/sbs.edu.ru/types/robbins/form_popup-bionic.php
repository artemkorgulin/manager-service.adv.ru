...
$tickets_count = [
	"day1"=>isset($_REQUEST['food-tickets_count2']) && $_REQUEST['food-tickets_count2'] > 0 ? $_REQUEST['food-tickets_count2']*1 : 0
];
...
if(true){
	if( !isset($_REQUEST['payment-type']) && !isset($_REQUEST['company']) ){
		...
		<input name="food-tickets_count2" value="'.$tickets_count['day1'].'" type="hidden">
		<button class="form__button" name="payment-type-online">Оплата банковской картой</button>
		<br><br>
		<button class="form__button" name="payment-type-invoice">Выставить счет на оплату</button>
