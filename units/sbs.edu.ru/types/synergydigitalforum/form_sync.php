...
$ticketsCount = isset($_REQUEST['sync_count']) && $_REQUEST['sync_count'] > 0 ? $_REQUEST['sync_count']*1 : 1;
...
if(true){
	...
	if(!isset($_REQUEST['payment-type']) && !isset($_REQUEST['company']) ){
		...
		<input name="sync_count" value="'.$ticketsCount.'" type="hidden">
	...
	// шаг 2, выбран способ оплаты
		...
		if( isset($_REQUEST['payment-type-invoice']) ){
			...
			<div class="popup-form-inner">
				<input name="company" required placeholder="На кого будет выставлен счет" type="text" class="popup-form__input">
				<br><br>
...
function createOrder( $lead, $ticketsCount, $priceVariant, $promocode, $category, $invoice, $company, $productId ){
	...
	if (isset($responseApi_arr->response->link) && $responseApi_arr->response->link != '') {
		return '<br><br><br>
			...
			<iframe style="height: 721px;width:100%%" src="'.$responseApi_arr->response->link.'" ></iframe>
		';
	}
}
