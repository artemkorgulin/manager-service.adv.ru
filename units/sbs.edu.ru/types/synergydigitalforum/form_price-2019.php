...
$product_id = trim($_REQUEST['product__id']) ? trim($_REQUEST['product__id']) : 7170908;
...
$syncCount = isset($_REQUEST['sync_count']) && $_REQUEST['sync_count'] > 0 ? $_REQUEST['sync_count']*1 : 0;
$priceVariant =  444;
...
if(true){
	...
	if( !isset($_REQUEST['payment-type']) && !isset($_REQUEST['company']) ){
		...
		<input name="sync_count" value="'.$syncCount.'" type="hidden">
		...
		<br>
		<button class="button" name="payment-type-online">Оплата картой</button>
		<button class="button" name="payment-type-invoice">Выставить счет на оплату</button>
		';
	...
	// шаг 2, выбран способ оплаты
		...
		// выбрана оплата онлайн, создаем заказ
			...
			$sendsuccess = createOrder( $lead, $ticketsCount, $syncCount, $priceVariant, $promocode, $category, false, $company,$product_id );

		}

		else if( isset($_REQUEST['payment-type-wb']) ){

			$sendsuccess = createOrder( $lead, $ticketsCount, $syncCount, $priceVariant, $promocode, $category, false, $company,$product_id,false,true );
	...
	// шаг 3, введено название компании при оплате по счету
		...
		createOrder( $lead, $ticketsCount, $syncCount, $priceVariant, $promocode, $category, true, $company,$product_id );
...
function createOrder( $lead, $ticketsCount, $syncCount = 0, $priceVariant, $promocode, $category, $invoice, $company, $productId,$applepay,$webmoney ){
	...
	if ($syncCount > 0 && $category != 615) {
		$seatsRandSynchr = getSeatsRandom($syncCount, 229);
		$seatsRand = array_merge($seatsRand, $seatsRandSynchr);
		$ticketsCount += $syncCount;
	}
