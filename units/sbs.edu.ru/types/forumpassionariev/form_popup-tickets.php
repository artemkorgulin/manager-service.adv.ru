...
if (!isset($_REQUEST['payment-type']) && !isset($_REQUEST['company'])) {
    ...
    <button class="form__button" name="payment-type-online">' . $onlineButton . '</button>
    <br><br>' . $invoiceButton . '
...
// шаг 2, выбран способ оплаты
    ...
    // выбрана оплата онлайн, создаем заказ
        ...
        $sendsuccess = '<script>$(".buy-ticket-head.font-excn-bold").html("");$(".buy-ticket-head.font-excn-bold").removeClass("buy-ticket-head");</script>' . createOrder($lead, $ticketsCount, $priceVariant, $promocode, $category, false, $company, $product_id);
    }
}
...
function createOrder($lead, $ticketsCount, $priceVariant, $promocode, $category, $invoice, $company, $productId)
{
    ...
    $responseApi = cURLsend('https://api.1001tickets.org/events/88', $postData);
