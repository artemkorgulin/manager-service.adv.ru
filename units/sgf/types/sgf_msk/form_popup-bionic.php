...
if (true) {
	if (!isset($_REQUEST['payment-type']) && !isset($_REQUEST['company'])) {
		...
		<br><br>
		<button class="button button6" name="payment-type-online">Оплата банковской картой</button><br><br>
		<button class="button button6"  name="payment-type-invoice" >Выставить счет на оплату</button>';
...
function createOrderFood($lead, $ticketsCount, $priceVariant, $promocode, $category, $invoice, $company, $productId)
{
	...
	if (isset($responseApi_arr->response->link) && $responseApi_arr->response->link != '') {
		return '<br><br><div class="font-size-24 font-bold uppercase color-blue">Оплата: Bionic Food TB (' . $ticketsCount . ')</div>
