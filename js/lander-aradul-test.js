...
var LanderJS = (function() {
	...
	LANDER.inputMasks = function() {
		if (typeof $.fn.inputmask == 'undefined') {
			LANDER.loadJS( 'http://cdnjs.cloudflare.com/ajax/libs/jquery.inputmask/3.3.3/jquery.inputmask.bundle.min.js', LANDER.inputMasks );
			return;
		}
		...
		$('form').not('#mse2_form, .nolander').each(function(){
			...
			$form.validate({
				...
				submitHandler: function (form) {
					...
					var owa_visitorId2 = LANDER.get_cookie('owa_v');
					var owa_sessionId2 = LANDER.get_cookie('owa_s');
					...
					if (owa_visitorId) $(form).append('<input type="hidden" name="owa_visitorId" value="' + owa_visitorId2 + '">');
					if (owa_sessionId) $(form).append('<input type="hidden" name="owa_sessionId" value="' + owa_sessionId2 + '">');
					if (PAPVisitorId)  $form.append('<input type="hidden" name="PAPVisitorId" value="' + PAPVisitorId + '">');
					if (roistat_visit) $form.append('<input type="hidden" name="roistat_visit" value="' + roistat_visit + '">');
					if (analytics_id)  $form.append('<input type="hidden" name="analytics_id" value="' + analytics_id + '">');
	...
	LANDER.privacy = function() {
		...
		if (!$.fn.fancybox) {
			LANDER.loadJS( 'http://cdnjs.cloudflare.com/ajax/libs/fancybox/2.1.5/jquery.fancybox.min.js', InitFancyPrivacy );
			$('head').append('<link href="http://cdnjs.cloudflare.com/ajax/libs/fancybox/2.1.5/jquery.fancybox.min.css" rel="stylesheet">');
		} else {
			InitFancyPrivacy();
		}
	};
